                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="w-12 h-12 flex-shrink-0">
                      <img 
                        src={`/variation-images/${recipe.outputVariation?.toLowerCase().replace(/\s+/g, '_').replace(/'/g, '').replace(/\?/g, 'q').replace(/[^\w_-]/g, '_')}.png`}
                        alt={recipe.outputVariation}
                        className="w-full h-full object-contain rounded"
                        onError={(e) => {
                          e.currentTarget.style.display = 'none';
                          e.currentTarget.parentElement!.innerHTML = `<div class="w-full h-full bg-gray-800 rounded flex items-center justify-center text-xl">🔧</div>`;
                        }}
                      />
                    </div>
