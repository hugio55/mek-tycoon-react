 ...
 * Used by sync system to fetch campaign details
 */
...
/**
 * Update NFT status in inventory
 * Used by sync system to apply updates from NMKR
 */
export const updateNFTStatus = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!nft) {
      throw new Error(`NFT with UID ${args.nftUid} not found in inventory`);
    }
    ...
    console.log(`[CAMPAIGN] Updated NFT ${nft.name} status: ${nft.status} → ${args.status}`);
