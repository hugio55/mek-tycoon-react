...
// DIAGNOSTIC: Check what's actually in the database right now
export const getDatabaseRawSettings = query({
  args: {},
  handler: async (ctx) => {
    const settings = await ctx.db
      .query("landingDebugUnified")
    ...
    if (!settings) {
      return { exists: false, message: "No settings in database" };
    }
    ...
    return {
      ...
      desktopSample: {
        logoSize: settings.desktop?.logoSize,
        starScale: settings.desktop?.starScale,
        bgStarCount: settings.desktop?.bgStarCount,
      },
      mobileSample: {
        logoSize: settings.mobile?.logoSize,
        starScale: settings.mobile?.starScale,
        bgStarCount: settings.mobile?.bgStarCount,
      },
      sharedSample: {
        selectedFont: settings.shared?.selectedFont,
        designVariation: settings.shared?.designVariation,
      },
      fullDesktop: settings.desktop,
      fullMobile: settings.mobile,
      fullShared: settings.shared,
    };
  },
});
