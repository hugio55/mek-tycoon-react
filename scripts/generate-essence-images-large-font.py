...
def create_essence_image(variation_name, variation_type, output_path):
    """Create a single essence image with a circle and LARGE text"""
    ...
    # Find the longest word to determine font size
    words = variation_name.split()
    longest_word = max(words, key=len) if words else variation_name

    # Try different font sizes to find the largest that fits
    font_size = 40  # Start with a large size
    font = None
    ...
    while font_size > 8:
        ...
        # Test if the longest word fits in the width
        bbox = draw.textbbox((0, 0), longest_word, font=font)
        text_width = bbox[2] - bbox[0]

        if text_width <= 115:  # Leave a little margin
            break
        font_size -= 2
    ...
    for word in words:
        ...
        if text_width > 115 and current_line:
    ...
    # If still too many lines, try to fit on fewer lines
    if len(lines) > 3:
        # For really long names, split more aggressively
        lines = []
        if len(words) <= 3:
    ...
    for i, line in enumerate(lines):
        ...
        for offset in [(2, 2), (1, 1), (1, 2), (2, 1)]:
            draw.text((text_x + offset[0], text_y + offset[1]), line,
...
# Output directory
output_dir = r"C:\Users\Ben Meyers\Documents\Mek Tycoon\TYCOON REACT 8-27\mek-tycoon-react\public\essence-images\120px mock"

# Create all essence images
print(f"Generating {len(variations)} essence images with large fonts...")
...
print(f"Successfully generated all {len(variations)} essence images with large fonts!")