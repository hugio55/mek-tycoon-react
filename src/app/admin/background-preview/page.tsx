...
// This is an isolated full-screen preview of the GlobalBackground
// It renders the exact same elements as the main GlobalBackground component
export default function BackgroundPreviewPage() {
  const [mounted, setMounted] = useState(false);

  // Exact same star generation as GlobalBackground
  const backgroundStars = useMemo(() => {
    ...
    return [...Array(100)].map((_, i) => ({
      ...
      top: `${rng.random() * 100}%`,
      size: rng.random() * 2 + 1,
      opacity: (rng.random() * 0.6 + 0.6) * 1.15,
      twinkle: rng.random() > 0.5,
      delay: rng.random() * 4,
    }));
  }, []);

  // Exact same particle generation as GlobalBackground
  ...
  // Exact same satellite generation as GlobalBackground
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50">
        <div className="bg-black/80 border border-yellow-500/50 rounded-lg px-6 py-3 text-center backdrop-blur-sm">
          <div className="text-yellow-400 font-bold uppercase tracking-wider text-sm mb-1">
            Universal Background Preview
          </div>
          <div className="text-gray-400 text-xs">
            100 Stars | 12 Yellow Particles | 4 Satellites | Industrial Grid
          </div>
        </div>
      </div>
