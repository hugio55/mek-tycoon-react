    ...
    <div className={`min-h-screen ${viewMode === 'split-view' ? 'flex' : 'bg-gray-900 p-3'}`}>
      ...
      <div
        ...
        style={{
          ...
          marginTop: 'auto',
          paddingTop: `${config.socialIconPaddingTop}px`,
          paddingBottom: `${config.socialIconPaddingBottom}px`,
        }}
      >
        {/* Backdrop blur layer */}
        {config.footerBlurAmount > 0 && (
          <div
            className="absolute inset-0 bg-gray-900/80"
            style={{
              backdropFilter: `blur(${config.footerBlurAmount}px)`,
              WebkitBackdropFilter: `blur(${config.footerBlurAmount}px)`,
            }}
          />
        )}

        {/* Edge feathering gradient */}
        {config.footerEdgeFeathering > 0 && (
          <div
            className="absolute left-0 right-0 pointer-events-none"
            style={{
              top: `-${config.footerEdgeFeathering}px`,
              height: `${config.footerEdgeFeathering}px`,
        ...
        <a
          ...
          style={{
            ...
            left: '50%',
            marginLeft: `-${50 * config.oeLogoScale}px`,
            zIndex: 10,
          }}
        >
          <img
        ...
        <div
          ...
          style={{
            top: `${config.socialIconVerticalPosition}%`,
            left: '50%',
            transform: 'translateX(-50%)',
            gap: `${config.socialIconGap}px`,
            zIndex: 10,
          }}
        >
          {/* Discord Icon */}
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="cursor-pointer transition-all duration-200 hover:scale-110 hover:brightness-125 active:scale-95"
          >
            <svg
              width={40 * config.socialIconScale}
              height={40 * config.socialIconScale}
              viewBox="0 0 24 24"
              fill="currentColor"
              style={{ color: config.socialIconColor }}
            >
