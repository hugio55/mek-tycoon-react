...
import { testTreeData } from "./test-tree-data";
...
export default function StoryClimbPage() {
  ...
  useEffect(() => {
    ...
    if (!storyData && existingSaves.length === 0) {
      console.log('No saved data found. Using test tree data...');
      
      // Use the imported test tree data instead of creating inline
      storyData = testTreeData;
      
      // Save it to localStorage for next time
      localStorage.setItem('savedStoryModes', JSON.stringify([testTreeData]));
      setSavedStoryModes([testTreeData]);
      
      /* Original inline data creation commented out
  ...
  }, [dbStoryTrees, currentNodeIndex, canvasWidth]); // Re-run when database trees load or canvas width changes
        ...
        <div className="flex-1 flex flex-col px-8">
          ...
          <div className="bg-gray-900/20 flex justify-center relative overflow-hidden" style={{ height: `${canvasHeight}px` }}>
            ...
            >
              ...
              {/* Draw nodes */}
              {!isDataLoaded && (
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-yellow-400 text-center">
                  <div className="text-xl mb-2">Loading Story Tree...</div>
                  <div className="text-sm text-gray-400">If this persists, check browser console for errors</div>
                </div>
              )}
              {nodes.length === 0 && isDataLoaded && (
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-yellow-400 text-center">
                  <div className="text-xl mb-2">No Story Tree Found</div>
                  <div className="text-sm text-gray-400">Create one in the Talent Builder first</div>
                </div>
              )}
              {nodes.map(node => {
