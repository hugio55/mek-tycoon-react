...
export default function TalentBuilderPage() {
  ...
  useEffect(() => {
    ...
    if (savedData) {
      try {
        ...
        // Ensure all three START nodes exist
        const hasHeadsStart = loadedNodes.some((n: TalentNode) => n.id === 'start-heads');
        const hasBodiesStart = loadedNodes.some((n: TalentNode) => n.id === 'start-bodies');
        const hasTraitsStart = loadedNodes.some((n: TalentNode) => n.id === 'start-traits');
      ...
      // Create three start nodes for heads, bodies, and traits
      ...
      setNodes(startNodes);
      // Center view on middle start node
      setPanOffset({ x: -1200, y: 0 });
    }
  }, []);
  ...
  const deleteNode = (nodeId: string) => {
    // Prevent deleting any START node
  ...
  const handleNodeClick = (nodeId: string, e: React.MouseEvent) => {
    ...
    if (mode === 'connect') {
      ...
      // Don't jump the node, just select it
    ...
    <div className="min-h-screen p-5 relative z-10">
      <div className="flex gap-4">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          <label className="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded transition-colors cursor-pointer">
            Import JSON
              ...
              <button
                onClick={async () => {
                  try {
                    ...
                    console.log('Templates might already exist');
                  }
                }}
