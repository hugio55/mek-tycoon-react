  ...
  const handleVerifyOwnership = async () => {
    ...
    try {
      ...
      const progressMsg = meks.length > 200
        ? `Querying ${meks.length} NFTs on-chain... (Large collection - may take up to 60s)`
      ...
      // Special handling for specific error types - Mobile friendly messages
      if (error.message?.includes('timed out')) {
        setVerificationError(
          `Verification timed out. Large collections (200+ NFTs) may require up to 60 seconds. ` +
          `Please wait 30 seconds and retry. If this persists after 3 attempts, contact support.`
        );
      } else if (error.message?.includes('query') || error.message?.includes('mutation') || error.message?.includes('ctx')) {
        setVerificationError(
          `Database error during verification. This is temporary. Please wait 10 seconds and retry.`
        );
        ...
        setVerificationError(error.message || 'Verification failed. Please retry in a moment.');
      }
    } finally {
      setIsVerifying(false);
      onVerificationEnd?.();
    ...
    <div className="mek-card-industrial mek-border-sharp-gold p-4 sm:p-6 space-y-3 sm:space-y-4 relative">
      ...
      {verificationError && (
        <div className="relative overflow-hidden rounded-lg border-2 border-red-500/50 bg-black/60 backdrop-blur-sm">
          ...
          <div className="mek-overlay-hazard-stripes h-10 sm:h-8 flex items-center px-3 sm:px-4 border-b border-red-500/30">
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="relative">
                <div className="w-7 h-7 sm:w-6 sm:h-6 bg-red-500/20 rounded-full flex items-center justify-center border border-red-500">
                  <span className="text-red-400 text-lg sm:text-lg font-bold leading-none">!</span>
                </div>
          ...
          <div className="p-3 sm:p-4 space-y-3">
            <p className="text-red-300 text-sm sm:text-sm leading-relaxed">
              {verificationError}
            </p>
      ...
      {/* Loading overlay when verifying - Mobile Optimized Full Screen Overlay */}
      {isVerifying && (
        <div className="absolute inset-0 bg-black/90 backdrop-blur-sm rounded-lg flex items-center justify-center z-50 p-4">
          <div className="text-center space-y-4 sm:space-y-6 max-w-md w-full">
            {/* Progress Spinner with Percentage - Larger on Mobile */}
            <div className="relative mx-auto w-28 h-28 sm:w-24 sm:h-24">
              ...
              <div className="absolute inset-0 flex items-center justify-center text-yellow-500 font-bold text-xl sm:text-lg">
                {progressPercent}%
              </div>
            </div>
            ...
            {/* Status Messages */}
            <div className="space-y-2 sm:space-y-3">
              <p className="text-yellow-500 font-bold text-lg sm:text-xl px-2" style={{ fontFamily: "'Orbitron', sans-serif" }}>
                VERIFYING OWNERSHIP
              </p>

              {/* Progress Message - Prominent on Mobile */}
              <div className="min-h-[60px] sm:min-h-[48px] flex items-center justify-center px-3">
                <p className="text-gray-300 text-base sm:text-base font-medium animate-pulse text-center leading-snug">
                  {verificationProgress || 'Initializing...'}
                </p>
              </div>

              {/* Time Estimate - More Prominent on Mobile */}
              {progressPercent < 100 && (
                <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-3 sm:p-2">
                  <p className="text-yellow-400 text-sm sm:text-xs font-medium text-center leading-relaxed">
                    {meks.length > 200
                      ? `Processing ${meks.length} NFTs - This may take up to 60 seconds`
                      : `Processing ${meks.length} NFTs - Please wait up to 30 seconds`}
                  </p>
                </div>
              )}

              {/* Mobile-Specific "Still Working" Indicator */}
              {progressPercent > 25 && progressPercent < 100 && (
                <div className="flex items-center justify-center gap-2 text-gray-500 text-xs sm:text-xs">
                  <div className="w-2 h-2 bg-yellow-500 rounded-full animate-pulse" />
                  <span className="uppercase tracking-wide">Still working...</span>
